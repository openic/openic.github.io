/* 
	...
	#10.04.2012 Serghei SN
*/
(function($) {	
 	
	// the available ease animation style
	var easeTypeArr = ['rollIn', 'fadeIn', 'fadeInUp', 'fadeInDown', 'fadeInLeft', 'fadeInRight', 'bounceIn', 'bounceInDown', 'bounceInUp', 'bounceInLeft', 'bounceInRight', 'rotateIn', 'rotateInDownLeft', 'rotateInDownRight', 'rotateInUpLeft', 'rotateInUpRight'];
    ...
    $.fn.oneByOne = function(options) {  
	    ...
	    // add the mobile touch screen support 
	   	_this.bind('touchstart', function (event) {
			...
			if(settings.naviShowOnHover)buttonCon.fadeIn();  				
			if(settings.naviShowOnHover)arrowButton.fadeIn();
			return false;
		});                
		...
		_this.bind('touchend', function (event) {
			...
			xdiff = 0; 
			if(settings.naviShowOnHover)buttonCon.delay(400).fadeOut();  				
			if(settings.naviShowOnHover)arrowButton.delay(400).fadeOut();
			return false;            
		})    
		...
		parent.mouseleave(function (event){
			if(settings.naviShowOnHover)buttonCon.fadeOut(); 
			if(settings.naviShowOnHover)arrowButton.fadeOut();
			if(settings.pauseByHover){
				startSlideShow();	
				isPlaying = true; 
			}
		...
		if(settings.showArrow){
  		...
  		if(settings.naviShowOnHover)buttonCon.hide(); 
		if(settings.naviShowOnHover)arrowButton.hide();
